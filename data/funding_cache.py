...
from data.market_models import FundingUpdate, MarketScope
from data.market_state import MarketStateStore


FundingRecord = FundingUpdate
...
class FundingCache:
    """State-driven funding cache facade. Direct apply only; no EventBus subscriptions."""
    ...
    def __init__(
        ...
        *,
        ...
        self._logger = get_logger(__name__, service=service_name, event_type="funding_cache")
    ...
    def register(self) -> None:
        ...
        self._logger.info("FundingCache registered in state-driven mode | raw_eventbus_subscriptions=0")
    ...
    def _register_cleanup_job(self) -> None:
        ...
        self._cleanup_job_id = self.scheduler.add_interval_job(
            name="funding-cache-state-cleanup",
    ...
    async def update(self, event: Mapping[str, Any] | FundingUpdate) -> None:
        ok = await self.ingestion.ingest_funding(event)
        if ok:
            self._metrics["records_stored"] += 1
        else:
            self._metrics["invalid_events"] += 1

    async def apply_funding(self, event: Mapping[str, Any] | FundingUpdate) -> None:
    ...
    async def get_latest(self, *, exchange: str, symbol: str, market_type: str = "usdm_futures") -> dict[str, Any] | None:
        ...
        if snapshot.funding is None:
            return None
        return {**snapshot.scope.to_dict(), **snapshot.funding.to_dict(), "current_price": snapshot.current_price}
    ...
    async def get_window_stats(self, *, exchange: str, symbol: str, market_type: str = "usdm_futures", **_: Any) -> dict[str, Any]:
        latest = await self.get_latest(exchange=exchange, symbol=symbol, market_type=market_type)
        return {"records": 1 if latest else 0, "latest": latest, "latest_funding_rate": None if latest is None else latest.get("funding_rate")}
    ...
    async def clear_symbol(self, *, exchange: str, symbol: str, market_type: str = "usdm_futures") -> int:
        ...
        if state is None or state.funding is None:
            return 0
        state.funding = None
        return 1
    ...
    async def size(self, *, exchange: str | None = None, symbol: str | None = None, market_type: str = "usdm_futures") -> int:
        ...
        return int(stats.get("funding_ingested", 0) or 0)
