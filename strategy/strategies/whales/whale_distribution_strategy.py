...
class WhaleDistributionPayload:
    """
    Normalized strategy-level payload для whale distribution.
...
@dataclass(slots=True)
class WhaleDistributionStrategyConfig(WhalesStrategyConfig):
    """
    Unified whale distribution strategy config.

    Strategy idea:
    - sell-side whale activity наростає після/біля highs або перед downside continuation;
    - pressure/cluster підтримують ask-side distribution;
    ...
    block_buy_side_pressure: bool = True
    ...
    tag_whale_distribution: str = "whale_distribution"
    tag_sell_side_distribution: str = "sell_side_distribution"
    tag_pre_breakdown_positioning: str = "pre_breakdown_positioning"
    tag_cluster_resistance: str = "cluster_resistance"
    ...
    distribution_rr_hint: float | None = None
    ...
    def validate(self) -> None:
        ...
        unit_fields = {
            "min_distribution_score": self.min_distribution_score,
            "min_distribution_confidence": self.min_distribution_confidence,
        ...
        hint_fields = {
            ...
            "distribution_rr_hint": self.distribution_rr_hint,
        }
        ...
        for attr in (
            "tag_whale_distribution",
            "tag_sell_side_distribution",
            "tag_pre_breakdown_positioning",
            "tag_cluster_resistance",
...
class WhaleDistributionStrategy(WhalesTradingStrategy):
    """
    Unified whale distribution strategy.
    ...
    def __init__(
        ...
        *,
        ...
        self.distribution_config: WhaleDistributionStrategyConfig = (
    ...
    def metadata(self) -> StrategyMetadata:
        return StrategyMetadata(
            ...
            tags=[
                ...
                self.distribution_config.tag_whale_distribution,
                self.distribution_config.tag_sell_side_distribution,
                self.distribution_config.tag_pre_breakdown_positioning,
                "analytics_whales",
            ],
            version="2.0.0",
            description=(
                "Interprets sell-side whale distribution / pre-breakdown positioning "
            ...
            metadata={
                ...
                "strategy_type": "whale_distribution",
    ...
    async def generate_signal(
        ...
        if not self.has_any_whales_data(
            context,
            tuple(self.distribution_config.required_whales_features),
        ):
            return None
        ...
        if is_stale(
            ...
            stale_after_seconds=self.distribution_config.stale_feature_max_age_seconds,
        ):
            return None

        if not self.accepts_whale_snapshot(
            payload.snapshot,
            require_futures_market_type=self.distribution_config.require_futures_market_type,
            min_confidence=self.distribution_config.min_confidence,
        ):
            return None

        if not self._passes_distribution_filters(payload):
            return None
        ...
        if breakdown.score < self.distribution_config.min_distribution_score:
            return None

        if breakdown.confidence < self.distribution_config.min_distribution_confidence:
            return None
        ...
        reasons = list(
            dict.fromkeys(
                [
                    "whale_distribution_signal",
                    f"side:{payload.side.value}",
                    f"distribution_side:{payload.distribution_side}",
                    *payload.reasons,
                    *breakdown.reasons,
                ]
            )
        ...
        metadata = {
            ...
            "distribution_side": payload.distribution_side,
        ...
        return self.build_whale_signal(
            ...
            confidence=breakdown.confidence,
            score=breakdown.score,
            setup_type=self.distribution_config.default_setup_type,
            reasons=reasons,
            confirmations=confirmations,
            source_features=source_features,
            metadata=metadata,
            priority=self.distribution_config.default_priority,
        )
    ...
    def _extract_payload(
        ...
        distribution_side = self._resolve_distribution_side(snapshot)
        if distribution_side != "sell":
            return None

        side = side_label_to_signal_side(distribution_side)
        ...
        reasons = [
            "whale_distribution_context",
            f"distribution_side:{distribution_side}",
        ...
        return WhaleDistributionPayload(
            snapshot=snapshot,
            side=side,
    ...
    def _resolve_distribution_side(
        self,
        snapshot: WhaleCompositeSnapshot,
    ) -> str:
        for candidate in (
    ...
    def _required_inputs(self) -> tuple[str, ...]:
        ...
        if self.distribution_config.require_pressure_confirmation:
            required.append("pressure")

        if self.distribution_config.require_cluster_confirmation:
            required.append("cluster")

        return tuple(required)

    def _passes_distribution_filters(
        self,
        ...
        if self.distribution_config.require_sell_side:
            if payload.distribution_side != "sell":
                return False

        if self.distribution_config.block_buy_side_pressure:
            if snapshot.dominant_side == "buy" or snapshot.whale_side == "buy":
                return False
        ...
        if self.distribution_config.require_activity_confirmation:
            ...
            if snapshot.total_notional < self.distribution_config.min_activity_notional:
                return False

            if snapshot.trade_count < self.distribution_config.min_activity_trade_count:
                return False
        ...
        if self.distribution_config.require_pressure_confirmation:
            ...
            if snapshot.imbalance_ratio < self.distribution_config.min_pressure_imbalance_ratio:
                return False

            if snapshot.pressure_score < self.distribution_config.min_pressure_score:
                return False
        ...
        if self.distribution_config.require_cluster_confirmation:
            ...
            if cluster_score < self.distribution_config.min_cluster_score:
                return False

        if self.distribution_config.require_continuation_confirmation:
            continuation_probability = snapshot.continuation_probability or 0.0
            if continuation_probability < self.distribution_config.min_continuation_probability:
                return False

        if self.distribution_config.block_high_exhaustion_probability:
            exhaustion_probability = snapshot.exhaustion_probability
            if exhaustion_probability is not None:
                if exhaustion_probability > self.distribution_config.max_exhaustion_probability:
                    return False

        if self.distribution_config.require_activity_same_side and snapshot.has_activity:
            activity_side = normalize_label(
                snapshot.activity.get("side")
                or snapshot.activity.get("dominant_side")
            )
            if activity_side in {"buy", "sell"} and activity_side != payload.distribution_side:
                return False

        if self.distribution_config.require_pressure_same_side and snapshot.has_pressure:
            if snapshot.dominant_side in {"buy", "sell"}:
                if snapshot.dominant_side != payload.distribution_side:
                    return False

        if self.distribution_config.require_cluster_same_side and snapshot.has_cluster:
            if snapshot.cluster_side in {"buy", "sell"}:
                if snapshot.cluster_side != payload.distribution_side:
                    return False

        if self.distribution_config.require_large_trade_same_side and snapshot.has_large_trade:
            large_trade_side = normalize_label(
                snapshot.large_trade.get("side")
                or snapshot.large_trade.get("trade_side")
            )
            if large_trade_side in {"buy", "sell"} and large_trade_side != payload.distribution_side:
                return False
    ...
    def _build_score_breakdown(
        ...
        *,
        ...
        activity_component = whale_activity_score(
            snapshot.activity,
            min_notional=self.distribution_config.min_activity_notional,
            min_trade_count=self.distribution_config.min_activity_trade_count,
        )
        pressure_component = whale_pressure_score(
            snapshot.pressure,
            min_imbalance_ratio=self.distribution_config.min_pressure_imbalance_ratio,
        )
        cluster_component = cluster_context_score(
            inputs,
            min_cluster_score=self.distribution_config.min_cluster_score,
        )
        continuation_component = continuation_context_score(
            inputs,
            min_continuation_probability=self.distribution_config.min_continuation_probability,
        )
        large_trade_component = (
            large_trade_score(
                snapshot.large_trade,
                min_notional=self.distribution_config.min_large_trade_notional,
                min_zscore=self.distribution_config.min_large_trade_zscore,
            )
            if self.distribution_config.use_large_trade_confirmation
            else 0.0
        )
        exhaustion_component = low_exhaustion_score(
            inputs,
            max_exhaustion_probability=self.distribution_config.max_exhaustion_probability,
        )
        ...
        weights = {
            "activity": self.distribution_config.score_activity_weight,
            "pressure": self.distribution_config.score_pressure_weight,
            "cluster": self.distribution_config.score_cluster_weight,
            "continuation": self.distribution_config.score_continuation_weight,
            "large_trade": self.distribution_config.score_large_trade_weight,
            "exhaustion": self.distribution_config.score_exhaustion_weight,
            "freshness": self.distribution_config.score_freshness_weight,
        }
        ...
        confidence = confidence_from_components(
            ...
            freshness=freshness_component,
            primary_weight=self.distribution_config.confidence_primary_weight,
            context_weight=self.distribution_config.confidence_context_weight,
            confirmation_weight=self.distribution_config.confidence_confirmation_weight,
            freshness_weight=self.distribution_config.confidence_freshness_weight,
        )
        ...
        if snapshot.total_notional >= (
            self.distribution_config.min_activity_notional
            * self.distribution_config.strong_activity_multiplier
        ):
            score += self.distribution_config.strong_activity_bonus
            confirmations.append("strong_whale_activity")

        if snapshot.imbalance_ratio >= self.distribution_config.strong_pressure_threshold:
            score += self.distribution_config.strong_pressure_bonus
            confirmations.append("strong_sell_side_pressure")

        if cluster_component >= 1.0:
            score += self.distribution_config.cluster_confirmation_bonus
            confirmations.append("cluster_resistance_confirms_distribution")

        if (
            snapshot.continuation_probability is not None
            and snapshot.continuation_probability >= self.distribution_config.strong_continuation_threshold
        ):
            score += self.distribution_config.continuation_confirmation_bonus
            confirmations.append("continuation_bias_confirmed")

        if large_trade_component > 0.0:
            score += self.distribution_config.large_trade_confirmation_bonus
            confirmations.append("large_trade_distribution_confirmation")

        if (
            snapshot.exhaustion_probability is None
            or snapshot.exhaustion_probability <= self.distribution_config.low_exhaustion_threshold
        ):
            score += self.distribution_config.low_exhaustion_bonus
            confirmations.append("low_exhaustion_risk")

        if snapshot.dominant_side == payload.distribution_side:
            confirmations.append("pressure_same_as_distribution_side")

        if snapshot.cluster_side == payload.distribution_side:
            confirmations.append("cluster_same_as_distribution_side")

        activity_side = normalize_label(
            snapshot.activity.get("side")
            or snapshot.activity.get("dominant_side")
        )
        if activity_side == payload.distribution_side:
            confirmations.append("activity_same_as_distribution_side")

        large_trade_side = normalize_label(
            snapshot.large_trade.get("side")
            or snapshot.large_trade.get("trade_side")
        )
        if large_trade_side == payload.distribution_side:
            confirmations.append("large_trade_same_as_distribution_side")
    ...
    def _tags(
        ...
        if payload.snapshot.cluster_score is not None:
            tags.append(self.distribution_config.tag_cluster_resistance)

        if payload.snapshot.continuation_probability is not None:
            tags.append(self.distribution_config.tag_continuation_bias)

        if (
            payload.snapshot.exhaustion_probability is None
            or payload.snapshot.exhaustion_probability <= self.distribution_config.low_exhaustion_threshold
        ):
            tags.append(self.distribution_config.tag_low_exhaustion)
    ...
    def _execution_hints(self) -> dict[str, Any]:
        ...
        return {
            ...
            "take_profit_bps": self.distribution_config.execution_take_profit_bps_hint,
            "risk_reward": self.distribution_config.distribution_rr_hint,
        }