...
class TrendConfig(BasePriceActionConfig):
    ...
    event_namespace: str = "price_action.trend"
...
class TrendAnalyzer(BasePriceActionModule[TrendState]):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            service_name="price_action.trend",
        )
    ...
    def _refresh_layer(self, layer: StructureLayer) -> List[TrendSignal]:
        ...
        layer_state.is_accelerating = strength >= self.config.acceleration_threshold and abs(slope_score) > 0.45
        ...
        layer_state.is_aligned_with_structure = self._is_direction_aligned_with_structure(direction, layer)
        ...
        if previous_direction != direction and direction in {TrendDirection.BULLISH, TrendDirection.BEARISH}:
            signals.append(
                self._create_signal(
                    layer=layer,
        ...
        if previous_strength >= self.config.trend_strength_threshold and strength < self.config.trend_strength_threshold:
            signals.append(
                self._create_signal(
                    layer=layer,
        ...
        if previous_direction in {TrendDirection.BULLISH, TrendDirection.BEARISH} and direction in {
    ...
    def _refresh_global_state(self) -> None:
        ...
        self._state.higher_timeframe_alignment = self._unit_score(self._higher_timeframe_alignment())
    ...
    def _structure_score(self, layer: StructureLayer) -> float:
        ...
        confidence = float(layer_ctx.get("confidence", 0.0))
        trend_strength = float(layer_ctx.get("trend_strength", 0.0))
    ...
    def _support_resistance_score(self, layer: StructureLayer) -> float:
        ...
        # ближче до support -> bullish, ближче до resistance -> bearish
    ...
    def _reversal_risk(
        ...
        short_return = abs((closes[-1] - closes[-self.config.short_window]) / max(closes[-self.config.short_window], 1e-9))
        medium_return = abs((closes[-1] - closes[-self.config.medium_window]) / max(closes[-self.config.medium_window], 1e-9))
    ...
    def _exhaustion_score(self, closes: Sequence[float], direction: TrendDirection) -> float:
    ...
    def _pullback_depth(self, closes: Sequence[float], direction: TrendDirection) -> float:
        if len(closes) < self.config.short_window:
            return 0.0
        if direction == TrendDirection.NEUTRAL:
            return 0.0
    ...
    def _continuation_probability(
        ...
        *,
        ...
        raw = strength * 0.5 + (1.0 - reversal_risk) * 0.25 + (1.0 - exhaustion_score) * 0.15 + (1.0 - consolidation_score) * 0.10
        return self._clamp_unit(raw)
    ...
    def _resolve_regime(
        ...
        *,
        ...
        if direction == TrendDirection.NEUTRAL:
            return TrendRegime.CONSOLIDATING if consolidation_score >= 0.55 else TrendRegime.UNKNOWN
    ...
    def _higher_timeframe_alignment(self) -> float:
        ...
        return self._clamp_unit(float(mtf.get("alignment_score", 0.0)))

    def _is_direction_aligned_with_structure(self, direction: TrendDirection, layer: StructureLayer) -> bool:
        structure_score = self._structure_score(layer)
        if direction == TrendDirection.BULLISH:
    ...
    def _extract_distance_pct(self, level_ctx: Any, current_price: float) -> Optional[float]:
    ...
    def _create_signal(
        ...
        *,
        ...
        signal = TrendSignal(
            ...
            direction=direction,
            strength=self._clamp_unit(strength),
        ...
        self._emit_event(
            self._build_event_name(signal.event_type.value),
    ...
    def _unit_score(self, value: float) -> float:
        return self._clamp_unit(value)

    def _signed_score(self, value: float) -> float:
        return self._clamp_signed(value)

    def _signal_to_dict(self, signal: TrendSignal) -> Dict[str, Any]:
        return self._safe_serialize(signal)
